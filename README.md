...
AutoCamTracker 是一個以影片、螢幕區域、webcam 或 iPhone 作為輸入的車輛偵測、單車追蹤、數位構圖與 GID 重新辨識工具。V1.7 加入 iPhone 實體相機變焦控制、15 FPS iPhone 串流、自動連線與啟動、ReID 快取與預測走廊、DockKit 安全控制，以及 Fixed Cut／AI Tracking／In-Out Auto 賽道拍攝模式；本版也修正 Before / After preview 抖動、iOS fallback timeout 誤觸發，以及雲台置中不足的預設控制參數。
...
- Before / After preview 使用固定 Canvas 尺寸與左上角錨點更新，避免每幀狀態文字或影像尺寸造成 UI 抖動。
...
- Vehicle Database 使用固定右側欄；ReID 模型位於 Tracking 主區塊，門檻與 Feature Mode 收在可展開的 Advanced ReID settings。
...
- `src/autocamtracker/main.py`：目前 V1.7 桌面版 Tkinter app 啟動入口。
...
- `src/autocamtracker/tracking/feature_gallery.py`：Master / Pending / Candidate gallery、ReID matching、crop quality filter。
...
- `src/autocamtracker/server/websocket_server.py`：V1.7 桌面端與 DockKit iOS app 的 WebSocket bridge。
...
2. V1.7 預設選擇 `iphone`，會在 `8765` port 啟動 WebSocket Server 並自動啟動影像管線；畫面會顯示可連線網址與 Copy 按鈕。
3. iOS App 會使用已保存的 URL 自動連線；需要更換 Mac 時，也可在連線頁輸入 `ws://<Mac 位址>:8765/ws/tracking` 後按 `Connect`。
4. iOS App 會以最高約 15 FPS 傳送 JPEG 相機畫面給 V1.7；桌面端完成 YOLO、BoT-SORT 與 GID ReID 後，再透過相同 WebSocket 回傳 tracking command 與實體相機 `zoom_factor`。
5. DockKit System Tracking 會由 iOS App 自動關閉，避免手機內建人物追蹤與電腦辨識同時搶控制權。
6. V1.7 iOS App 使用 2 秒 safety timeout，重複序號或單筆 JSON decode failure 不會立刻觸發 fallback STOP；雲台預設 dead-zone、速度與 smoothing 也調整為更容易收斂到中心。
...
- V1.7 仍以單一車輛追蹤與互動式 GID 管理為主。
- 實體變焦已接到 iOS 相機端；若 telemetry 的 `camera_display_zoom_factor` 仍不動，優先檢查 iOS App 是否為 1.7、是否收到 `zoom_factor`、以及相機硬體可用倍率範圍。
- 若手機 log 出現 DockKit fallback failure，V1.7 會避免同一 fallback failure 每幀重複回報造成桌面狀態抖動。
...
- 若 GID 已被舊版本寫入錯車 feature，V1.7 會防止繼續污染，但不會自動清理既有髒資料。
- Identity DB 是本機資料，預設位於 `outputs/vehicle_identity.sqlite3`，不應作為 release 檔案上傳。
- `outputs/`、`.venv/`、cache、測試輸出都不屬於乾淨 release 內容。

---

# AutoCamTracker V1.7 English

AutoCamTracker is a vehicle detection, single-target tracking, digital reframing, and GID re-identification tool. V1.7 adds physical iPhone camera zoom control, a 15 FPS iPhone stream, automatic connection and startup, cached and trajectory-gated ReID, safer DockKit control, racetrack Fixed Cut, AI Tracking, and In/Out Auto shot modes, plus steadier Before / After previews and less twitchy iOS fallback handling.
